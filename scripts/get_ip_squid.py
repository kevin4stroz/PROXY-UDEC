...
def getIp():
    try:
        cursor = db.cursor()
        cursor.execute('SELECT distinct(ip) FROM db_proxy.logs_squid')
...
def validateIfExist(ipv4):
    try:
        ...
        cursor.execute( 'SELECT * FROM db_proxy.country_ip WHERE db_proxy.country_ip.ip = %s \
                        AND db_proxy.country_ip.logs_privoxy_idlogs_privoxy is NULL',(ipv4,))
...
def insertGeoIp(jsonResponse):
    try:
        ...
        cursor = db.cursor()
        cursor.execute('SELECT idlogs_squid FROM db_proxy.logs_squid \
                        WHERE db_proxy.logs_squid.ip = %s',jsonResponse['ip'])
